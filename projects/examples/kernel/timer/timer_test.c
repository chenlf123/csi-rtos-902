...
void example_main(void)
{
    ...
    if (timer01 != NULL && timer02 != NULL) {
        printf("create Timer successfully\n");
    }

    if (!(csi_kernel_timer_start(timer01, 1000))) {
    ...
    if (csi_kernel_timer_stop(timer01) == 0) {
    ...
    if (!(csi_kernel_timer_del(timer01))) {
        printf("delete Timer1 successfully\n");
    }

    csi_kernel_timer_start(timer02, 100);
    printf("start Timer2\n");
    csi_kernel_delay(1000);
    csi_kernel_timer_stop(timer02);
    csi_kernel_timer_del(timer02);
