...
static gpio_test_t gpio_config_cases[] = {
    {GPIO_MODE_PULLNONE, GPIO_DIRECTION_OUTPUT + 1, NULL, (CSI_DRV_ERRNO_GPIO_BASE | GPIO_ERROR_DIRECTION)},
};

static gpio_test_t gpio_pin_irq_set_cases[] = {
    {NULL, NULL, GPIO_IRQ_MODE_HIGH_LEVEL + 1, (CSI_DRV_ERRNO_GPIO_BASE | GPIO_ERROR_IRQ_MODE)},
};
...
void test_gpio_rising_edge()
{
    ...
    ASSERT_TRUE(ret == 0);
    printf("gpio rising_edge test successfully!!!\n");
}

void test_gpio_falling_edge()
{
    uint32_t ret = 0;
    gpio_pin_handle_t pin = NULL;
    test_pin_gpio_init();
...
void test_gpio_high_level()
{
    ...
    ret = csi_gpio_pin_uninitialize(pin);
    ASSERT_TRUE(ret == 0);
    printf("gpio high level test successfully!!!\n");
}

void test_gpio_low_level()
{
    uint32_t ret = 0;
    gpio_pin_handle_t pin = NULL;
    test_pin_gpio_init();
    ...
    ret = csi_gpio_pin_uninitialize(pin);
    ASSERT_TRUE(ret == 0);
    printf("gpio low level test successfully!!!\n");
}

void test_gpio_get_value()
{
    uint32_t ret = 0;
    bool value = 1;
    gpio_pin_handle_t pin = NULL;
    ...
    ret = csi_gpio_pin_uninitialize(pin);
    ASSERT_TRUE(ret == 0);
    printf("test gpio get value is pass\r\n");
}
