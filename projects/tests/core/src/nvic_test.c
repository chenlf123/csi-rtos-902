...
void user1_int_irq_handler(void)
{
    ...
    if (VIC_Active) {
        printf("User1  VIC_Active successfully!\n");
    }
    ...
    if (VIC_wakeup1) {
        printf("User1 VIC_SetWakeupIRQ successfully!\n");
    }
    ...
    if (VIC_Pend) {
        printf("User2 VIC_SetPending successfully!\n");
    }

    printf("--------User1 irq handle successfully!\n");
}
...
void user2_int_irq_handler(void)
{
    ...
    if (VIC_Active) {
        printf("User2  VIC_Active successfully!\n");
    }
    ...
    if (VIC_wakeup2) {
        printf("User2 VIC_SetWakeupIRQ successfully!\n");
    }
    ...
    if (VIC_Pend) {
        printf("User3 VIC_SetPending successfully!\n");
    }

    printf("--------User2 irq handle successfully!\n");
}
void user3_int_irq_handler(void)
{
    ...
    if (VIC_Active) {
        printf("User3  VIC_Active successfully!\n");
    }
    ...
    if (VIC_wakeup3) {
        printf("User3 VIC_SetWakeupIRQ successfully!\n");
    }

    csi_vic_clear_pending_irq(USER3_IRQ);
    csi_vic_disable_irq(USER3_IRQ);
    printf("--------User3 irq handle successfully!\n");
}
...
void user1_int(IRQn_Type IRQn, uint32_t priority)
{
    ...
    if (VIC_Priority == USER1_INT_PRIO) {
        printf("User1 VIC_SetPriority IRQ successfully!\n");
    }
...
void user2_int(IRQn_Type IRQn, uint32_t priority)
{
    ...
    if (VIC_Priority == USER2_INT_PRIO) {
        printf("User2 VIC_SetPriority IRQ successfully!\n");
    }
...
void user3_int(IRQn_Type IRQn, uint32_t priority)
{
    ...
    if (VIC_Priority == USER3_INT_PRIO) {
        printf("User3 VIC_SetPriority IRQ successfully!\n");
    }
...
int test_vic_test(void)
{
    ...
    while (1) {
        if (tick_count > 10000) {
            ...
            if (VIC_Pend) {
                printf("User1 VIC_SetPending successfully!\n");
            }
    ...
    if (!VIC_wakeup) {
        printf("User1 VIC_ClearWakeupIRQ successfully!\n");
    }
    ...
    if (!VIC_wakeup) {
        printf("User2 VIC_ClearWakeupIRQ successfully!\n");
    }
    ...
    if (!VIC_wakeup) {
        printf("User3 VIC_ClearWakeupIRQ successfully!\n");
    }
